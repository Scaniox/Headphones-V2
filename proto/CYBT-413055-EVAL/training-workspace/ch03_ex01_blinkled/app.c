...
{
//	wiced_hal_gpio_configure_pin(WICED_PLATFORM_LED_2, GPIO_OUTPUT_ENABLE, GPIO_PIN_OUTPUT_LOW);
    while( 1 )
    {
//		wiced_rtos_delay_milliseconds( SLEEP_250MS, ALLOW_THREAD_TO_SLEEP );
//		wiced_hal_gpio_set_pin_output(WICED_PLATFORM_LED_2, GPIO_PIN_OUTPUT_HIGH);

        /* Send the thread to sleep for a specified number of milliseconds */
        wiced_rtos_delay_milliseconds( SLEEP_250MS, ALLOW_THREAD_TO_SLEEP );
//        wiced_hal_gpio_set_pin_output(WICED_PLATFORM_LED_2, GPIO_PIN_OUTPUT_LOW);
    }
}
